...
export default function LamboScene() {
  return (
    // <Canvas gl={{ logarithmicDepthBuffer: true, antialias: false }} dpr={[1, 1.5]} camera={{ position: [0, 0, 15], fov: 25 }}>
      <>
      <color attach="background" args={['#15151a']} />
      <Lamborghini rotation={[0, Math.PI / 1.5, 0]} scale={0.2} />
      <hemisphereLight intensity={0.5} />
      <ContactShadows resolution={1024} frames={1} position={[0, -1.16, 0]} scale={15} blur={0.5} opacity={1} far={20} />
      <mesh scale={53} position={[45, -16, -1.5]} rotation={[-Math.PI / 2, 0, Math.PI / 2.5]}>
        <ringGeometry args={[0.9, 1, 4, 1]} />
        <meshStandardMaterial color="white" roughness={0.75} />
      </mesh>
      <mesh scale={53} position={[-30, -16, -1]} rotation={[-Math.PI / 2, 0, Math.PI / 2.5]}>
