...
function AnimatedCamera() {
    ...
    const start = new THREE.Vector3(0, 100, 0) // Top view
    const end = new THREE.Vector3(100, 30, 30)  // Right view
    ...
    useFrame(() => {
      if (progress.current < 1) {
        progress.current += 0.01 // Adjust speed here
...
function AsteroidBelt() {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const radius = THREE.MathUtils.randFloat(24 , 27) // Between Mars and Jupiter
    const x = Math.cos(angle) * radius * 2
    const z = Math.sin(angle) * radius * 2
    const y = THREE.MathUtils.randFloatSpread(0.5) // slight vertical spread
